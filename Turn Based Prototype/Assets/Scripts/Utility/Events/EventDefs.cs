...
namespace Utility.Events
{
    ...
    public abstract class GenericEvent<T, T1, T2, T3> : GenericEvent<T, T1, T2>
    {
        public GenericEvent() { }
        public GenericEvent(T first, T1 second, T2 third, T3 fourth) : base (first, second, third)
        {
            Fourth = fourth;
        }

        public T3 Fourth { get; private set; }
        public void Set(T first, T1 second, T2 third, T3 fourth) { base.Set(first, second, third); Fourth = fourth; }
    }
}
